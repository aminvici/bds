...
func newUpdateMetaDataJob(splitter *ETCSplitter) *updateMetaDataJob {
...
func (j *updateMetaDataJob) run() error {
	...
	err := db.Find(&metas)
	if err != nil {
		log.Error("worker etc: job '%s' get table list from meta error", j.name)
		return err
	}
	//update meta table
	for _, meta := range metas {
		...
		if err != nil {
			log.Error("worker etc: job %s get table %s count from meta error", j.name, meta.Name)
			log.DetailError(err)
			continue
		}
		if len(result) == 0 {
		...
		if err != nil {
			log.Error("worker etc: job '%s' get table %s id from meta error", j.name, meta.Name)
			log.DetailError(err)
			continue
		}
		//update meta
		for _, v := range result {
			...
			if err != nil {
				log.Error("worker etc: job '%s' update table %s meta error", j.name, meta.Name)
				log.DetailError(err)
				continue
			}
	...
	log.Debug("worker etc: job '%s' elasped time %s", j.name, elaspedTime.String())
	return nil
}
...
func newGetBatchBlockJob(splitter *ETCSplitter) *getBatchBlockJob {
...
func (j *getBatchBlockJob) run() error {
	...
	if err != nil {
		log.Error("worker etc: job '%s' get latest block error", j.name)
	}
	var start, end int64
	var old, free bool
	...
	if now.Sub(j.splitter.latestSaveDataTimestamp).Seconds() > 15 && now.Sub(j.splitter.latestReceiveMessageTimestamp) > 5 {
		log.Warn("worker etc: job '%s' splitter etc no message received or no data processing", j.name)
		free = true
	}

	if free && old {
		log.Info("worker etc: job '%s' get block range from %d to %d", j.name, start, end)
		err = j.splitter.remoteHandler.SendBatchBlock(start, end)
		if err != nil {
			log.Error("worker etc: job '%s' rpc call error", j.name)
			log.DetailError(err)
		}
...
func newRefreshContractAddressesJob(splitter *ETCSplitter) *refreshContractAddressesJob {
...
func (j *refreshContractAddressesJob) run() error {
	...
	sql := fmt.Sprintf("SELECT address FROM etc_account WHERE type = 1 OR type = 11")
	...
	if err != nil {
		log.Error("worker etc: job '%s' get contract account list from account error", j.name)
		return err
	}
	...
	for _, v := range result {
		...
		if !ok {
			log.Warn("worker etc: job '%s' insert address %s to filter error", j.name, address)
		}
	}
	oldCount := contractAddressFilter.Count()
	contractAddressFilter.Update(filter)
	log.Debug("worker etc: job '%s' contract addresses count %d:%d", j.name, oldCount, filter.Count())

	stats.Add(MetricCronWorkerJobRefreshContractAddresses, 1)
	elaspedTime := time.Now().Sub(startTime)
	log.Debug("worker etc: job '%s' account update elasped time %s", j.name, elaspedTime.String())
	return nil
}
