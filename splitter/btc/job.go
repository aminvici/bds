...
func (j *updateMetaDataJob) run() error {
	...
	err := db.Find(&metas)
	if err != nil {
		log.Error("worker btc: job %s get table list from meta error", j.name)
		return err
	}
	//update meta table
	for _, meta := range metas {
		...
		if err != nil {
			log.Error("worker btc: job %s get table %s count from meta error", j.name, meta.Name)
			log.DetailError(err)
			continue
		}
		if len(result) == 0 {
		...
		if err != nil {
			log.Error("worker btc: job %s get table %s id from meta error", j.name, meta.Name)
			log.DetailError(err)
			continue
		}
		//update meta
		for _, v := range result {
			...
			if err != nil {
				log.Error("worker btc: job %s update table %s meta error", j.name, meta.Name)
				log.DetailError(err)
				continue
			}
	...
	log.Debug("worker btc: job %s elasped time %s", j.name, elaspedTime.String())
	return nil
}
...
func newGetBatchBlockJob(splitter *BTCSplitter) *getBatchBlockJob {
...
func (j *getBatchBlockJob) run() error {
	...
	if err != nil {
		log.Error("worker btc: job '%s' get latest block error", j.name)
	}
	var start, end int64
	var old, free bool
	...
	if now.Sub(j.splitter.latestSaveDataTimestamp).Seconds() > 60 && now.Sub(j.splitter.latestReceiveMessageTimestamp) > 15 {
		log.Warn("worker btc: job '%s' splitter btc no message received or no data processing", j.name)
		free = true
	}

	if free && old {
		log.Info("worker btc: job '%s' get block range from %d to %d", j.name, start, end)
		err = j.splitter.remoteHandler.SendBatchBlock(start, end)
		if err != nil {
			log.Error("worker btc: job '%s' rpc call error", j.name)
			log.DetailError(err)
		}
